...
extern void MIYOO_PA_ENABLE(bool enable);
...
{
	lcdc_cmd(0x2c);//Start Memory Write
...
static void TFT_init(void)
{
    ...
    gpio_direction_output(SLCD_RESET, 1);
#if DO_LCD_RESET
	...
	printk("Display Id[85,85,52]: manID[%02XH] verID[%02XH] drvID[%02XH]\n", 
						id[2], id[3], id[4]);
    ...
    lcdc_dat(0x08);    // bp0 0x0a
    lcdc_dat(0x08);    // fp0 0x09

    lcdc_cmd(0xc3);
    lcdc_dat(0x01);    // line inversion
    lcdc_dat(0x00);    // division 0x00
    lcdc_dat(0x1a);    // clock 0x1a
    lcdc_dat(0x08);    // bp2 0x0a
    lcdc_dat(0x08);    // fp2 0x09
    ...
    mdelay(250);
#endif
    lcdc_cmd(0x36);//Memory Data Access Control
    lcdc_dat(0xB0);

    lcdc_cmd(0x3a);//Interface Pixel Format
    lcdc_dat(0x55);

    lcdc_cmd(0x2a);//Column Address Set
    lcdc_dat(0x00);
    lcdc_dat(0x00);
    lcdc_dat(0x01);
    lcdc_dat(0x3f);

    lcdc_cmd(0x2b);//Row Address Set
    lcdc_dat(0x00);
    lcdc_dat(0x00);
    lcdc_dat(0x00);
    lcdc_dat(0xef);

    lcdc_cmd(0xb2);//Porch Setting
    lcdc_dat(0x0a);// BPA
    lcdc_dat(0x0a);// FPA
    lcdc_dat(0x00);// PSEN disable
    lcdc_dat(0x33);
    lcdc_dat(0x33);

    lcdc_cmd(0xb7);//Gate Control
    lcdc_dat(0x35);// VGH 13.26V, VGL -10.43V

    lcdc_cmd(0xb8);//Gate On Timing Adjustment
    lcdc_dat(0x2f);
    lcdc_dat(0x2b);
    lcdc_dat(0x2f);

    lcdc_cmd(0xbb);//VCOM Setting
    lcdc_dat(0x15);//0.625

    lcdc_cmd(0xc0);//LCM Control
    lcdc_dat(0x3C);

    lcdc_cmd(0xc2);//VDV and VRH Command Enable
    lcdc_dat(0x01);
    ...
    lcdc_cmd(0xc6);//Frame Rate Control in Normal Mode
    lcdc_dat(0x09);//0x05=90HZ, 0x09=75HZ, 0x0F=60HZ, 0x15=50HZ, 0x1E=40HZ

    lcdc_cmd(0xd0);//Power Control 1
    lcdc_dat(0xa4);
    lcdc_dat(0xa1);

    lcdc_cmd(0xe8);//Power Control 2
    lcdc_dat(0x03);

    lcdc_cmd(0xe9);//Equalize time control
    lcdc_dat(0x0d);
    lcdc_dat(0x12);
    lcdc_dat(0x00);

    lcdc_cmd(0xe0);//Positive Voltage Gamma Control
    lcdc_dat(0x70);
    lcdc_dat(0x00);
    lcdc_dat(0x06);
    lcdc_dat(0x09);
    ...
    lcdc_cmd(0xe1);//Negative Voltage Gamma Control
    lcdc_dat(0xd0);
    lcdc_dat(0x00);
    lcdc_dat(0x06);
    lcdc_dat(0x09);
    ...
    lcdc_cmd(0x35);// Tearing Effect Line On
    lcdc_dat(0x00);// te mode 0:  V-Blanking information only
...
static void suniv_lcdc_init(struct myfb_par* par)
{
    ...
    total = par->mode.xres * 1 + h_front_porch + bp;//@ horizontal total time
...
static int myfb_remove(struct platform_device* dev)
{
    ...
    if(info) {
        ...
        for(i=0; i<LRAM_NUM; i++) {
            dma_free_coherent(&dev->dev, par->lram_size, par->lram_virt[i], par->lram_phys[i]);
        }
...
static int myfb_probe(struct platform_device* device)
{
    ...
    for(i=0; i<LRAM_NUM; i++) {
        par->lram_virt[i] = dma_alloc_coherent(&device->dev, par->lram_size, (resource_size_t*) &par->lram_phys[i], GFP_KERNEL | GFP_DMA);
        if(!par->lram_virt[i]) {
...
static long myioctl(struct file* filp, unsigned int cmd, unsigned long arg)
{
	...
	printk("fb0 myioctl cmd=0x%X\n", cmd);
    switch(cmd) {
    case MIYOO_FB0_PUT_OSD:
        break;
    case MIYOO_FB0_GET_VER:
        hw_ver = copy_to_user((void*)arg, &hw_ver, sizeof(uint32_t));
    break;
    }
    return 0;
}
...
MODULE_DESCRIPTION("Allwinner f1c500s framebuffer driver for miyoo TFT-LCD");
